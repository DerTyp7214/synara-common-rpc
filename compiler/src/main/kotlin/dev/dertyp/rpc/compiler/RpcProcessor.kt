...
class RpcProcessor(
    ...
    private fun generateRustBridge(resolver: Resolver, symbols: List<KSClassDeclaration>, dependencies: Dependencies) {
        ...
        file.use { out ->
            ...
            out.writeLine("    pub fn common_rpc_validate_server(ptr: *mut NativeRpcManager, host: *const c_char, port: c_int, path: *const c_char, use_ssl: bool) -> c_int;")
            out.writeLine("    pub fn common_rpc_update_auth(ptr: *mut NativeRpcManager, args: *const u8, len: c_int);")
            out.writeLine("    pub fn common_rpc_is_authenticated(ptr: *mut NativeRpcManager) -> bool;")
            out.writeLine("    pub fn common_rpc_get_auth_token(ptr: *mut NativeRpcManager) -> *mut c_char;")
            out.writeLine("    pub fn common_rpc_get_refresh_token(ptr: *mut NativeRpcManager) -> *mut c_char;")
            out.writeLine("    pub fn common_rpc_is_token_expired(ptr: *mut NativeRpcManager) -> bool;")
            out.writeLine("}")
            out.writeLine("")
            out.writeLine("pub struct ServerValidationResult {")
            out.writeLine("    pub validated: bool,")
            out.writeLine("    pub use_ssl: bool,")
            out.writeLine("}")
            out.writeLine("")
            ...
            out.writeLine("    }")
            out.writeLine("")
            out.writeLine("    pub fn set_url(&self, url: &str) {")
            ...
            out.writeLine("    }")
            out.writeLine("")
            out.writeLine("    pub fn validate_server(&self, host: &str, port: i32, path: Option<&str>, use_ssl: bool) -> ServerValidationResult {")
            out.writeLine("        let host_c = CString::new(host).unwrap();")
            out.writeLine("        let path_c = CString::new(path.unwrap_or(\"/\")).unwrap();")
            out.writeLine("        let res = unsafe { common_rpc_validate_server(self.manager, host_c.as_ptr(), port as c_int, path_c.as_ptr(), use_ssl) };")
            out.writeLine("        ServerValidationResult {")
            out.writeLine("            validated: res != 0,")
            out.writeLine("            use_ssl: res == 2,")
            out.writeLine("        }")
            out.writeLine("    }")
            out.writeLine("")
            out.writeLine("    pub fn update_auth(&self, response: &AuthenticationResponse) {")
            out.writeLine("        let arg_bytes = serde_cbor::to_vec(response).unwrap();")
            out.writeLine("        unsafe { common_rpc_update_auth(self.manager, arg_bytes.as_ptr(), arg_bytes.len() as c_int) };")
            out.writeLine("    }")
            out.writeLine("")
            out.writeLine("    pub fn is_authenticated(&self) -> bool {")
            out.writeLine("        unsafe { common_rpc_is_authenticated(self.manager) }")
            out.writeLine("    }")
            out.writeLine("")
            out.writeLine("    pub fn get_auth_token(&self) -> Option<String> {")
            out.writeLine("        let ptr = unsafe { common_rpc_get_auth_token(self.manager) };")
            out.writeLine("        if ptr.is_null() { return None; }")
            out.writeLine("        let s = unsafe { CStr::from_ptr(ptr).to_string_lossy().into_owned() };")
            out.writeLine("        unsafe { common_rpc_free_buffer(ptr as *mut u8) };")
            out.writeLine("        Some(s)")
            out.writeLine("    }")
            out.writeLine("")
            out.writeLine("    pub fn get_refresh_token(&self) -> Option<String> {")
            out.writeLine("        let ptr = unsafe { common_rpc_get_refresh_token(self.manager) };")
            out.writeLine("        if ptr.is_null() { return None; }")
            out.writeLine("        let s = unsafe { CStr::from_ptr(ptr).to_string_lossy().into_owned() };")
            out.writeLine("        unsafe { common_rpc_free_buffer(ptr as *mut u8) };")
            out.writeLine("        Some(s)")
            out.writeLine("    }")
            out.writeLine("")
            out.writeLine("    pub fn is_token_expired(&self) -> bool {")
            out.writeLine("        unsafe { common_rpc_is_token_expired(self.manager) }")
            out.writeLine("    }")
            out.writeLine("}")
